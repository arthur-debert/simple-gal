...
- Scaffolding for upcoming auto-reindex feature. Adds a new `[auto_indexing]` config section accepted (but not yet acted on) by `SiteConfig`: `auto` chooses when the build hook runs (`off` | `source_only` | `export_only` | `both`, default `off`), `spacing` sets the gap between consecutive indices as `10^spacing` (default `1` → step of 10), `padding` zero-pads the numeric prefix to a fixed width (default `3`). Validated bounds: `spacing ≤ 9`, `padding ≤ 12`. Setting this section today is a no-op — the `simple-gal reindex` CLI command and the build-pipeline hook that consumes it land in a follow-up PR. Full design in `docs/dev/auto-reindex.md`.
- Library scaffolding for the reindex feature in `src/reindex.rs`: `plan_reindex` is an order-preserving pure function that turns a list of numbered entries plus `spacing`/`padding` parameters into a minimal rename plan (unnumbered entries are skipped, no-op renames are filtered), and `apply_plan` executes a plan via a collision-safe two-phase rename (source → `.reindex-tmp-*` → target) with best-effort rollback on phase-1 failure. Phase-2 failures are reported loudly with the exact source/temp/target triple and leave `.reindex-tmp-*` files on disk so the user can recover manually; a subsequent `apply_plan` on a directory still containing stale temps refuses to proceed (`ApplyError::DirtyTemps`). Walker and CLI wiring to follow.
