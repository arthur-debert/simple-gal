...
- Data-model refactor Phase 4b (additive): `CanonicalImage` now carries content-derived metadata — `iptc_title`, `iptc_description`, `width`, `height`. Process populates these once per unique canonical_id during the decode pass (first ref wins via a per-id memo); later refs to the same byte-identical source reuse the cached values instead of re-reading IPTC and dimensions. Forwarded end-to-end through scan → process → generate so downstream consumers (including external tooling) can read content-addressed metadata directly. Per-ref `title` / `description` / `dimensions` on `OutputImage` are unchanged — this phase adds the canonical fields alongside; the removal of per-ref duplicates and the `MANIFEST_VERSION` bump to 2 are deferred to a follow-up.
