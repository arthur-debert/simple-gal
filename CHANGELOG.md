...
- Data-model refactor Phase 3: generate stage forwards the canonical-image view end-to-end (`OutputManifest.canonical_images` + `OutputImage.canonical_id`) and uses it to deduplicate the `/all-photos/` page. When the same byte-identical source appears in two or more albums, the All Photos grid now shows it exactly once, linked to the first album where it appears. Legacy manifests (no `canonical_id`) keep the prior behavior — every full-index thumbnail renders. The rest of generate's output (per-album pages, per-image pages, URLs) is unchanged.
- Data-model refactor Phase 4a: cache schema mismatches are now loud. The processing cache (`.cache-manifest.json` in the processed directory) has always carried a `version` field, but a mismatch used to silently drop the cache and proceed. Now `process` / `build` abort with a clear message naming the exact file and the expected vs found versions. A new `--auto-reset-cache` flag on `process` and `build` wipes the processed directory before load to recover automatically (only on version mismatch — IO / corruption errors bubble up unchanged). Matches the nuke-on-mismatch policy documented in `docs/dev/data-model-refactor.md` §5.2 — no data-migration code, clean rebuild instead. `CacheManifest::load_strict` returns the new `CacheLoadError` (Io / Corrupt / VersionMismatch); the lenient `load()` stays for tests and ad-hoc tooling.
- Data-model refactor Phase 4b (additive): `CanonicalImage` now carries content-derived metadata — `iptc_title`, `iptc_description`, `width`, `height`. Process populates these once per unique canonical_id during the decode pass via a per-key `Arc<Mutex<Option<_>>>` cell memo — same pattern used for source-hash dedup in Phase 2, so concurrent rayon workers on the same canonical_id serialize on the inner cell while workers on different ids stay parallel. The memo is checked **before** `get_dimensions` / `read_metadata`, so later refs to the same byte-identical source skip those calls entirely — a real IO win on sites that reuse images across galleries. Forwarded end-to-end through scan → process → generate so downstream consumers (including external tooling) can read content-addressed metadata directly. Per-ref `title` / `description` / `dimensions` on `OutputImage` still hold the pre-merged values; the removal of per-ref duplicates and the `MANIFEST_VERSION` bump to 2 are deferred to a follow-up.

### Changed
- Data-model refactor Phase 5: `[auto_indexing]` config simplified. The earlier four-value `auto` enum (`off` | `source_only` | `export_only` | `both`) is replaced by a single boolean `sync_source_files`. `source_only` and `both` had identical on-disk effect; `export_only` never had an implementation. The collapse keeps the one meaningful knob ("do I want source files renamed automatically?") and drops the three values that weren't pulling their weight. **Migration (breaking for users with a non-default `auto_indexing` config):** replace `auto = "source_only"` or `auto = "both"` with `sync_source_files = true`; drop `auto = "off"` (it's the default). Old configs fail with confique's unknown-field error that names `auto`, pointing at the new field. The `simple-gal reindex` CLI command is unchanged.
...
- New `simple-gal reindex [PATH]` command: normalize `NNN-` prefixes on albums, groups, pages, and images while preserving order. `--spacing N` controls the step (indices are spaced by `10^N`, default `1` → 10, 20, 30), `--padding W` zero-pads the prefix to a fixed width (default `3`), `--flat` limits reindex to the target directory instead of recursing into numbered subdirectories, `--dry-run` prints the plan without touching disk, and `--yes` / `-y` skips the TTY confirmation prompt. Interactive text-mode runs show the full `OLD → NEW` plan and prompt before applying; JSON-mode runs require `--yes` so a misconfigured script can't silently rewrite a content tree. Images and their `.txt` / `.md` sidecars rename together so the pairing scan relies on stays intact.
- New `[auto_indexing]` config section (`auto`, `spacing`, `padding`) supplies defaults for the command — CLI flags override `config.toml`, which overrides compiled defaults. `auto` accepts `off` (default) | `source_only` | `export_only` | `both` and is reserved for the upcoming build-pipeline hook (follow-up PR); setting it today is a no-op until that lands. Validated bounds: `spacing ≤ 9`, `padding ≤ 12`.
- Reindex library surface in `src/reindex.rs`: `plan_reindex` is a pure, order-preserving function that turns a numbered-entry list plus `spacing`/`padding` into the minimal rename plan (unnumbered entries skipped, no-op renames filtered); `apply_plan` executes the plan via a collision-safe two-phase protocol (source → `.reindex-tmp-*` → target) with best-effort rollback on phase-1 failure and loud, recoverable reporting on phase-2 failure (temp files kept on disk, exact source/temp/target named); `read_entries` / `reindex_tree` walk a directory or tree into `Vec<Entry>` + `DirReport`, skipping `config.toml` / `description.*` / hidden files everywhere and `{assets_dir}` + site-description file at root. Full design in `docs/dev/auto-reindex.md`.
- New `ErrorKind::Reindex` classification (exit code 9) so automation can branch on reindex failures specifically.
