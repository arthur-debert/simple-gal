...
- Data-model refactor Phase 4a: cache schema mismatches are now loud. The processing cache (`.cache-manifest.json` in the processed directory) has always carried a `version` field, but a mismatch used to silently drop the cache and proceed. Now `process` / `build` abort with a clear message naming the exact file and the expected vs found versions. A new `--auto-reset-cache` flag on `process` and `build` wipes the processed directory before load to recover automatically. Matches the nuke-on-mismatch policy documented in `docs/dev/data-model-refactor.md` §5.2 — no data-migration code, clean rebuild instead. `CacheManifest::load_strict` returns the new `CacheLoadError` (Io / Corrupt / VersionMismatch); the lenient `load()` stays for tests and ad-hoc tooling.
